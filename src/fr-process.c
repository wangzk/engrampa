...
fr_channel_data_flush (FrChannelData *channel)
{
	...
	/* In raw mode the tight loop can spin on G_IO_STATUS_AGAIN
	 * because no encoding errors break out.  Limit spinning to
	 * ~1 s total (200 × 5 ms). */
...
fr_process_detect_and_convert (FrProcess *process)
{
	...
	/*
	 * Build a charset-detection buffer from FILENAMES only.
	 *
	 * For most archive listing tools (unzip -ZTs, tar tf, 7z l, …)
	 * the filename is the LAST space-separated field on each output
	 * line.  Header / summary lines (Archive: …, Zip file size: …,
	 * 3 files …) produce short numeric or path-like last fields,
	 * which we skip.  Multiple filenames are collected so that
	 * uchardet sees enough CJK bytes to make a correct decision
	 * (rather than being fooled by the ASCII header).
	 */
	raw_buf = g_string_new ("");
	/* Extract last space-separated field from each raw line as the
	 * filename candidate, skipping header/summary boilerplate. */
	for (scan = process->out.raw; scan; scan = scan->next) {
		...
		if (last != NULL && strlen (last) > 8) {
			g_string_append (raw_buf, last);
			g_string_append_c (raw_buf, '\n');
		}
	}
	if (raw_buf->len == 0) {
